      ...
      {displayServices.length > 0 && (
        <div style={{ padding: compact ? '14px 14px 0' : '18px 16px 0', background: background_style === 'dark' ? '#0a0a12' : '#fff' }}>
          ...
          {services_layout === 'grid' ? (
            ...
            <div className="flex flex-col gap-2" style={{ opacity: !hasServices ? 0.45 : 1 }}>
              {displayServices.map((svc, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-2.5 rounded-xl overflow-hidden"
                    style={{
                      background: background_style === 'dark' ? '#13132a' : '#f9fafb',
                      border: `1px solid ${background_style === 'dark' ? '#1e1e3a' : '#f0f0f0'}`,
                      padding: compact ? '8px 10px' : '10px 12px',
                    }}>
                    {/* Thumbnail */}
                    <div className="flex-shrink-0 rounded-lg overflow-hidden"
                      style={{ width: thumbSize, height: thumbSize, background: primary_color + '18' }}>
                      {svc.image_url
                        ? <img src={svc.image_url} alt="" className="w-full h-full object-cover" />
                        : <div className="w-full h-full flex items-center justify-center">
                            <svg viewBox="0 0 24 24" style={{ width: compact ? 18 : 22, height: compact ? 18 : 22, fill: primary_color }}>
                              <path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 3c1.93 0 3.5 1.57 3.5 3.5S13.93 13 12 13s-3.5-1.57-3.5-3.5S10.07 6 12 6zm7 13H5v-.23c0-.62.28-1.2.76-1.58C7.47 15.82 9.64 15 12 15s4.53.82 6.24 2.19c.48.38.76.97.76 1.58V19z"/>
                            </svg>
                          </div>
                      }
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="font-bold truncate"
                        style={{ fontSize: compact ? 11 : 14, color: background_style === 'dark' ? '#e2e8f0' : '#1f2937' }}>{svc.title}</p>
                      {svc.description && <p className="truncate mt-0.5"
                        style={{ fontSize: compact ? 9 : 11, color: '#9ca3af' }}>{svc.description}</p>}
                    </div>
                    {/* Price + WA */}
                    <div className="flex flex-col items-end gap-1 flex-shrink-0">
                      ...
                      {svcWaLink && hasServices && !compact && (
                        <a href={svcWaLink} target="_blank" rel="noopener noreferrer"
                          ...
                          style={{ background: '#22c55e18', color: '#16a34a', fontSize: 10, padding: '3px 8px' }}>
                          <WhatsAppIcon size={9} /> הזמן
                        </a>
                      )}
                    </div>
                  </div>
