...
function ImagePicker({ label, value, onChange, aspect = 'square', botId }) {
  ...
  const handleFile = async (file) => {
    ...
    try {
      ...
      const path = `${botId}/${aspect}-${Date.now()}.${ext}`;
      const { error } = await supabase.storage.from('bot-images').upload(path, file, { upsert: true });
      if (error) throw error;
      const { data: { publicUrl } } = supabase.storage.from('bot-images').getPublicUrl(path);
      onChange(publicUrl);
    } catch (e) {
