...
export default function CardPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50" dir="rtl">
      ...
      <div className="hidden md:block fixed inset-0 pointer-events-none"
        style={{ backgroundImage: `radial-gradient(ellipse at 20% 80%, ${color}14 0%, transparent 50%), radial-gradient(ellipse at 80% 20%, ${color}0a 0%, transparent 50%)` }} />
      ...
      {/* ── DESKTOP layout ── */}
      <div className="hidden md:flex min-h-screen items-center justify-center px-8 py-12 gap-12 relative">
        ...
        >
          <div className="rounded-[2.5rem] overflow-hidden"
            style={{ boxShadow: `0 40px 100px -20px ${color}40, 0 12px 32px -8px rgba(0,0,0,0.15)` }}>
            <CardPreview data={card} compact={false} showActions={false} showSocial={false} />
          </div>
        </motion.div>

        {/* Right panel — CTA + info */}
        <motion.div
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
        ...
        >
          {/* Business info */}
          <div>
            <h1 className="text-3xl font-black text-gray-900 mb-1">{card.business_name}</h1>
            {card.description && <p className="text-gray-500 text-base leading-relaxed">{card.description}</p>}
          </div>

          {/* WA button */}
          {waLink && (
            <a href={waLink} target="_blank" rel="noopener noreferrer"
              className="flex items-center justify-center gap-3 py-4 rounded-2xl text-white font-black text-base transition-all hover:opacity-90 hover:scale-[1.02] active:scale-[0.98]"
              style={{ background: 'linear-gradient(135deg, #22c55e, #16a34a)', boxShadow: '0 8px 28px -4px rgba(34,197,94,0.5)' }}>
              <WAIcon size={20} /> קבע תור ב-WhatsApp
            </a>
          )}
          ...
          {card.phone && (
            <a href={`tel:${card.phone}`}
              className="flex items-center justify-center gap-2 py-3.5 rounded-2xl font-semibold text-sm border-2 border-gray-200 text-gray-700 bg-white hover:bg-gray-50 transition-colors"
              style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
              <PhoneIcon size={16} /> התקשר עכשיו
            </a>
          )}
          ...
          {(card.instagram || card.facebook || card.tiktok || card.location_url) && (
            <div className="flex gap-3">
              {card.instagram && <a href={`https://instagram.com/${card.instagram.replace('@','')}`} target="_blank" rel="noopener noreferrer" className="w-11 h-11 rounded-full flex items-center justify-center text-white hover:scale-110 transition-transform" style={{ background: '#E1306C', boxShadow: '0 4px 12px #E1306C55' }}><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg></a>}
              {card.facebook && <a href={card.facebook.startsWith('http') ? card.facebook : `https://facebook.com/${card.facebook}`} target="_blank" rel="noopener noreferrer" className="w-11 h-11 rounded-full flex items-center justify-center text-white hover:scale-110 transition-transform" style={{ background: '#1877F2', boxShadow: '0 4px 12px #1877F255' }}><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg></a>}
              {card.tiktok && <a href={`https://tiktok.com/@${card.tiktok.replace('@','')}`} target="_blank" rel="noopener noreferrer" className="w-11 h-11 rounded-full flex items-center justify-center text-white hover:scale-110 transition-transform" style={{ background: '#010101', boxShadow: '0 4px 12px #00000055' }}><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.32 6.32 0 00-.79-.05 6.34 6.34 0 00-6.34 6.34 6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.33-6.34V8.69a8.28 8.28 0 004.84 1.55V6.79a4.84 4.84 0 01-1.07-.1z"/></svg></a>}
              {card.location_url && <a href={card.location_url} target="_blank" rel="noopener noreferrer" className="w-11 h-11 rounded-full flex items-center justify-center text-white hover:scale-110 transition-transform" style={{ background: '#4285F4', boxShadow: '0 4px 12px #4285F455' }}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg></a>}
            </div>
          )}

          {/* Vizzit branding */}
          <a href="/" className="inline-flex items-center gap-2 px-4 py-2 rounded-2xl self-start transition-all hover:shadow-md"
